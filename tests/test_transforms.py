...
from src.augmentation.transforms import get_train_transforms, get_val_transforms
...
class TestAugmentationTransforms(unittest.TestCase):
    ...
    def test_get_train_transforms_output_is_float(self):
        ...
        self.assertIsInstance(transformed_tensor, torch.Tensor, "Output should be a torch.Tensor")
        self.assertEqual(transformed_tensor.dtype, torch.float32, "Tensor dtype should be float32")
        
        # Basic shape check: C, H, W. For grayscale, C=1.
        self.assertEqual(transformed_tensor.ndim, 3, "Tensor should have 3 dimensions (C, H, W)")
        self.assertEqual(transformed_tensor.shape[0], 1, "Channel dimension should be 1 for grayscale")
        self.assertEqual(transformed_tensor.shape[1], 32, "Height should match dummy image")
        self.assertEqual(transformed_tensor.shape[2], 32, "Width should match dummy image")
